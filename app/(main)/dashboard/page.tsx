  ...
  return (
    <div className="relative flex items-center justify-center" style={{ width: 120, height: 120 }}>
      <svg width={120} height={120} className="-rotate-90">
        ...
        <circle cx={60} cy={60} r={r} fill="none" stroke="#6366F1" strokeWidth={9}
...
export default async function DashboardPage() {
  ...
  // Weekly study data (last 7 days sessions)
  ...
  return (
    <div className="p-6 space-y-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-[#161B22] border border-white/10 rounded-2xl p-4 space-y-2">
          <div className="flex items-center gap-2 text-[#6366F1]">
            <span className="text-lg">⏱</span>
            <span className="text-xs font-medium text-slate-400">오늘 집중 시간</span>
          </div>
          <p className="text-2xl font-bold text-white font-mono">
            {Math.floor(todayMins / 60)}h {todayMins % 60}m
          </p>
          <p className="text-xs text-slate-500">목표 {goalMins / 60}시간</p>
        </div>

        <div className="bg-[#161B22] border border-white/10 rounded-2xl p-4 space-y-2">
          <div className="flex items-center gap-2 text-[#22C55E]">
            <span className="text-lg">🔥</span>
            <span className="text-xs font-medium text-slate-400">연속 집중</span>
          </div>
          <p className="text-2xl font-bold text-white font-mono">{profile?.streak_days ?? 0}일</p>
          <p className="text-xs text-slate-500">계속 유지하세요!</p>
        </div>

        <div className="bg-[#161B22] border border-white/10 rounded-2xl p-4 space-y-2">
          <div className="flex items-center gap-2" style={{ color: tier.color }}>
            <span className="text-lg">👑</span>
            <span className="text-xs font-medium text-slate-400">레벨</span>
          </div>
          <p className="text-2xl font-bold text-white font-mono">Lv.{progress.level}</p>
          <p className="text-xs text-slate-500">다음까지 {(progress.required - progress.current).toLocaleString()} XP</p>
        </div>

        <div className="bg-[#161B22] border border-white/10 rounded-2xl p-4 space-y-2">
          <div className="flex items-center gap-2 text-[#EAB308]">
            <span className="text-lg">⭐</span>
            <span className="text-xs font-medium text-slate-400">총 집중 시간</span>
          </div>
          <p className="text-2xl font-bold text-white font-mono">{totalHours}h {totalMins}m</p>
          <p className="text-xs text-slate-500">누적 기록</p>
        </div>
      </div>
                ...
                return (
                  <div key={i} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-500">+{ach.xp_reward} XP</p>
                    </div>
      ...
      <div>
        ...
        {publicRooms.length === 0 ? (
          <div className="bg-[#161B22] border border-white/10 rounded-2xl p-8 text-center text-slate-500">
            ...
            <Link href="/room/create" className="text-sm text-[#6366F1] hover:underline mt-2 inline-block">첫 번째로 만들어보세요 →</Link>
          </div>
        ) : (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/80 to-transparent">
                  ...
                  <div className="flex items-center justify-between mt-1">
                    ...
                    <span className={`text-xs px-1.5 py-0.5 rounded-full font-medium ${room.status === 'active' ? 'bg-[#22C55E]/30 text-[#22C55E]' : 'bg-white/20 text-white'}`}>
